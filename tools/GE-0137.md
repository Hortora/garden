---
id: GE-0137
title: "`git push` to a non-bare repo is rejected when the target branch is checked out"
type: gotcha
domain: tools
stack: "Git (all versions)"
tags: [branch, git, push, repo, tools, when]
score: 12
verified: true
staleness_threshold: 730
submitted: 2026-04-08
---

## `git push` to a non-bare repo is rejected when the target branch is checked out

**ID:** GE-0137
**Stack:** Git (all versions)
**Symptom:** Integration tests simulating concurrent git push/pull between two clones fail with:

```
remote: error: refusing to update checked out branch: refs/heads/main
remote: error: By default, updating the current branch in a non-bare repository
remote: error: is denied, because it will make the index and work tree inconsistent
```

**Context:** Tests that `git init` a directory and clone it twice, expecting both clones to push to the "origin". Fails because the origin has a working tree with `main` checked out.

### What was tried (didn't work)
```python
# Init, clone twice, have both push to origin
subprocess.run(["git", "clone", str(origin), str(session_a)])
subprocess.run(["git", "clone", str(origin), str(session_b)])
git(session_a, "push", "origin", "main")  # ✅
git(session_b, "push", "origin", "main")  # ❌ "refusing to update checked out branch"
```

### Root cause
When you push to a non-bare repository, git refuses to update the branch currently checked out in the target's working tree. This prevents the working tree from becoming out of sync with the index. The restriction applies even if the working tree is clean.

### Solution 1 — Use a bare repo as the shared remote
**Approach:** Replace the non-bare origin with `git init --bare`.
**Pros:** Eliminates the problem entirely — bare repos have no working tree, no checked-out branch.
**Cons/trade-offs:** Requires restructuring the test setup. Can't inspect files directly in the bare repo.

```python
bare = root / "origin.git"
subprocess.run(["git", "init", "--bare", str(bare)])

# Seed via a temporary clone
seed = root / "seed"
subprocess.run(["git", "clone", str(bare), str(seed)])
# ... make initial commit in seed, push to bare ...

# Both sessions clone from the bare remote
subprocess.run(["git", "clone", str(bare), str(session_a)])
subprocess.run(["git", "clone", str(bare), str(session_b)])
git(session_a, "push", "origin", "main")  # ✅
git(session_b, "push", "origin", "main")  # ✅ (or conflict if diverged — expected)
```

### Alternative — `receive.denyCurrentBranch=updateInstead`

When the target repo can't be bare (e.g., slot clones that push back to working repos), configure the target to accept pushes to the checked-out branch:

```bash
git -C <target-repo> config receive.denyCurrentBranch updateInstead
```

**Approach:** Atomically updates the ref, index, AND working tree on the target. Refuses if the working tree is dirty (safe — won't overwrite uncommitted work).

**Pros:** Target repo stays a normal working repo — no bare conversion needed. The working tree is always consistent with the pushed ref. Idempotent — setting it again is a no-op.

**Cons/trade-offs:** Config is permanent on the target repo (local only, can be unset). Any push to the checked-out branch is accepted — not just from the intended source.

**Key behavior:** When main IS checked out, `updateInstead` handles the push (ref + working tree + index). When main is NOT checked out (repo is on a feature branch), the push to main works by default — `denyCurrentBranch` doesn't fire for non-checked-out branches. Together, this means pushing to main works regardless of which branch the target has checked out.

### Why non-obvious
The error message says "refusing to update checked out branch" — sounds like branch protection, not a bare vs non-bare distinction. Developers familiar with GitHub (which uses bare repos internally) never encounter this in normal work. It only surfaces in test environments using a local directory as the "remote." The fix is simple but requires knowing "bare" is the right concept.

**See also:** GE-20260422-75b92e — git checkout/rebase on a branch checked out in a worktree fails

*Score: 12/15 · Included because: error message actively misleads, affects anyone writing git integration tests, bare repo concept not prominent in learning materials · Reservation: none*

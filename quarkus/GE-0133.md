...
## Variant — Quarkus extension runtime JARs you build yourself

The same problem occurs when you build a Quarkus extension and embed it as a dependency in another Quarkus application. The extension's runtime JAR is treated as a third-party jar by the embedding app — its beans will not be discovered without a Jandex index.

**Fix:** Add `jandex-maven-plugin` to the extension's runtime module POM:
...
**Symptom in this context:** `UnsatisfiedResolutionException` on a bean declared in your extension's runtime module, when your extension is used as a `<dependency>` in another Quarkus app. The app's own beans compile and wire fine; only the extension's beans are missing.

**Verified on:** quarkus-ledger 1.0.0-SNAPSHOT, quarkus-qhorus 1.0.0-SNAPSHOT embedded in Claudony (Quarkus 3.32.2)

...
## Variant — External library removes an API class; tests pass from cached `.class`

**Context:** A locally-developed dependency is updated (e.g. `mvn install` of
`quarkus-ledger`) and removes or renames a class that the consuming project
previously compiled against. `mvn test` passes because `target/classes` still
holds the old `.class` from the previous compilation. `mvn clean test` fails
with a compilation error on the now-removed class.

**Why dangerous:** The failure mode is inverted — you get **false green** (tests
pass) rather than a runtime error. The removed class is silently used from cache;
the mismatch is only detectable on a full clean build.

**Trigger:** Any `mvn install` of a locally-developed dependency whose public API
may have changed since the last clean build of the consuming project.

**Fix:** Always run `mvn clean test` (not just `mvn test`) after installing a
locally-developed library. Consider adding a CI rule: any PR that modifies a
local dependency triggers a clean build downstream.

...
**Date:** 2026-04-07
**Submission ID:** GE-0089
**Type:** technique
**Source project:** sparge (mdproctor.github.io blog migrator)
**Session context:** Making inline element adjacency visible in the HTML prettifier for the source editor
**Suggested target:** `tools/html2text.md` (same file as GE-0082, GE-0088 — or `python/beautifulsoup.md`)
...
**What it achieves:** Preserves information about which HTML elements were adjacent to non-whitespace content in the original DOM, surviving through `prettify()` which would otherwise destroy that information by adding newlines everywhere.
**Context:** BS4 `prettify()` adds newlines around every tag including inline elements. After prettify, `<b>Name</b>(text)` and `<b>Name</b> (text)` look the same in the output (both have `</b>` and `(text)` on different lines). Marking the following sibling with U+2060 before prettify lets you rejoin them afterward.
...
# Step 5: Clean up any remaining markers (nested cases)
content = content.replace(MARKER, '')
```

Result: `<b>Name</b>(text)` on one line (adjacent) vs `<b>Name</b>` and `(text)` on separate lines (space existed) — the adjacency is now visually unambiguous in the editor.

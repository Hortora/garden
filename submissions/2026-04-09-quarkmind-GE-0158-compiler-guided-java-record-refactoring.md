...
**Key insight for immutable records replacing themselves:** any method that creates a new `Unit` from an existing one (e.g. inside `List.replaceAll()`) must carry ALL fields through, including the new ones. Missing `u.shields()` in such a replacement silently resets the field to its default on every invocation.
...
**Applied to:** Adding `shields`/`maxShields` to the `Unit` record across 20 files (6 production, 14 test). All 236 tests passed after the refactor with no behaviour change.

**Suggested target:** `java/records.md`
